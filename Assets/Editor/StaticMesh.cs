...
public class StaticMesh : Editor
{
    ...
    public static void SplitScene()
    {
        ...
        string sceneFolder = "Assets/StaticMesh/" + EditorSceneManager.GetActiveScene().name;
        if (!Directory.Exists("Assets/StaticMesh")) Directory.CreateDirectory("Assets/StaticMesh");
        ...
        for (int i = 0; i < meshesToCreate.Count; i++)
        {
            ...
            // Ensure unique mesh name in memory
            string baseName = meshFilters[i].name;
            if (!nameCounters.ContainsKey(baseName)) nameCounters[baseName] = 0;
            string uniqueName = baseName + "_" + nameCounters[baseName]++;
            meshesToCreate[i].name = uniqueName;

            // Ensure unique asset path
            string assetPath = AssetDatabase.GenerateUniqueAssetPath(sceneFolder + "/" + uniqueName + ".asset");
            AssetDatabase.CreateAsset(meshesToCreate[i], assetPath);
        }
