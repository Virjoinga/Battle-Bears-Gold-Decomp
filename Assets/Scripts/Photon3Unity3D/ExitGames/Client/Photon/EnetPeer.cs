				...
				if (!base.IsSendingOnlyAcks && timeInt > timeoutInt && sentReliableCommands.Count > 0)
				{
					lock (sentReliableCommands)
					{
						...
						while (queue.Count > 0)
						{
							...
							if ((int)debugOut >= 3)
							{
								base.Listener.DebugReturn(DebugLevel.INFO, string.Format("Resending command: {0}. times out: {1} now: {2}", current, current.roundTripTimeout, timeInt));
							}
						}
					}
				}
		...
		internal void SendData(byte[] data, int length)
		{
			try
			{
				...
				if (base.NetworkSimulationSettings.IsSimulationEnabled)
				{
					...
					{
						rt.SendUdpPackage(dataCopy, length);
					});
				}
				else
				{
