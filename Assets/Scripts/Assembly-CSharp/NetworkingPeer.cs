...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	public Action<PhotonPlayer> OnPropertiesUpdated;
	...
	private void ReadoutProperties(ExitGames.Client.Photon.Hashtable gameProperties, ExitGames.Client.Photon.Hashtable pActorProperties, int targetActorNr)
	{
		...
		if (targetActorNr > 0)
		{
			...
			if (playerWithID != null)
			{
				...
				{
					OnPropertiesUpdated(playerWithID);
				}
			}
			return;
		}
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		case 252:
			if (OnPropertiesUpdated != null)
			{
				OnPropertiesUpdated(mLocalActor);
			}
			break;
		case 251:
		{
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		{
			mGameList = new Dictionary<string, RoomInfo>();
			ExitGames.Client.Photon.Hashtable hashtable5 = (ExitGames.Client.Photon.Hashtable)photonEvent.Parameters[222];
			foreach (DictionaryEntry item in hashtable5)
			{
				string text4 = (string)item.Key;
				mGameList[text4] = new RoomInfo(text4, (ExitGames.Client.Photon.Hashtable)item.Value);
			}
		...
		case 229:
		{
			...
			foreach (DictionaryEntry item2 in hashtable3)
			{
				...
				Room room = new Room(text, (ExitGames.Client.Photon.Hashtable)item2.Value);
		...
		case byte.MaxValue:
		{
			...
			if (mActors[num] == mLocalActor)
			{
				...
				{
					if (mLocalActor.ID != num4 && !mActors.ContainsKey(num4))
					{
						Debug.Log("creating player");
						AddNewPlayer(num4, new PhotonPlayer(false, num4, string.Empty));
					}
				}
		...
		case 96:
			...
			{
				if (parameter.Value is ExitGames.Client.Photon.Hashtable)
				{
					PhotonManager.Instance.CreateRemoteUser((ExitGames.Client.Photon.Hashtable)parameter.Value);
					break;
				}
			}
			break;
		case 104:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter2 in photonEvent.Parameters)
			{
				if (parameter2.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter2.Value;
					break;
				}
			}
			if (hashtable != null)
			{
				NetSyncManager.Instance.DispatchNetSyncAction((long)hashtable[(byte)72], (byte)hashtable[(byte)95], (ExitGames.Client.Photon.Hashtable)hashtable[(byte)96], (int)hashtable[(byte)83]);
			}
			break;
		case 97:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter3 in photonEvent.Parameters)
			{
				if (parameter3.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter3.Value;
					break;
				}
			}
			if (hashtable != null)
			{
				NetSyncManager.Instance.DispatchNetSyncState((long)hashtable[(byte)72], (int)hashtable[(byte)80], (int)hashtable[(byte)83]);
			}
			break;
		case 105:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter4 in photonEvent.Parameters)
			{
				if (parameter4.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter4.Value;
					break;
				}
			}
			if (hashtable != null)
			{
				NetSyncManager.Instance.DispatchSyncFireProjectile((long)hashtable[(byte)72], (float)hashtable[(byte)98], (float)hashtable[(byte)99], (float)hashtable[(byte)100], (float)hashtable[(byte)101], (float)hashtable[(byte)102], (float)hashtable[(byte)103], (int)hashtable[(byte)83]);
			}
			break;
		case 93:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter5 in photonEvent.Parameters)
			{
				if (parameter5.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter5.Value;
					break;
				}
			}
			if (hashtable != null && !NetSyncManager.Instance.ContainsNetSyncObject((long)hashtable[(byte)72]))
			{
				PhotonManager.Instance.CreateNetSyncObject((long)hashtable[(byte)72], (string)hashtable[(byte)81], (int)hashtable[(byte)80], (int)hashtable[(byte)82], (float)hashtable[(byte)73], (float)hashtable[(byte)74], (float)hashtable[(byte)75], (float)hashtable[(byte)76], (float)hashtable[(byte)77]);
			}
			break;
		case 94:
			Debug.Log("EventCode.UnregisterNetSyncObject");
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter6 in photonEvent.Parameters)
			{
				if (parameter6.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter6.Value;
				}
			}
			if (hashtable != null)
			{
				Debug.Log("EventCode.UnregisterNetSyncObject ing...");
				if (NetSyncManager.Instance != null)
				{
					NetSyncManager.Instance.UnregisterNetSyncObject((long)hashtable[(byte)72]);
				}
			}
			break;
		case 92:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter7 in photonEvent.Parameters)
			{
				if (parameter7.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter7.Value;
				}
			}
			if (hashtable != null)
			{
				NetSyncManager.Instance.DispatchNetSyncTransform((long)hashtable[(byte)72], (float)hashtable[(byte)73], (float)hashtable[(byte)74], (float)hashtable[(byte)75], (float)hashtable[(byte)76], (float)hashtable[(byte)77], (int)hashtable[(byte)83]);
			}
			break;
		case 109:
			foreach (KeyValuePair<byte, object> parameter8 in photonEvent.Parameters)
			{
				if (parameter8.Value is int)
				{
					PhotonManager.Instance.numObjectsToCreate = (int)parameter8.Value;
					break;
				}
			}
			break;
		case 123:
			hashtable = null;
			foreach (KeyValuePair<byte, object> parameter9 in photonEvent.Parameters)
			{
				if (parameter9.Value is ExitGames.Client.Photon.Hashtable)
				{
					hashtable = (ExitGames.Client.Photon.Hashtable)parameter9.Value;
				}
			}
			if (hashtable != null)
			{
				long netID = (long)hashtable[(byte)72];
				string name = (string)hashtable[(byte)97];
				int state = (int)hashtable[(byte)80];
				NetSyncManager.Instance.RegisterNetSyncServerObject(netID, name, state);
			}
			break;
		case 106:
		{
			string text2 = (string)PhotonNetwork.room.customProperties["levelName"];
			CumulativeStats.Instance.OnSaveStats();
			string text3 = text2 + ":";
			PhotonPlayer[] playerList = PhotonNetwork.playerList;
			foreach (PhotonPlayer photonPlayer2 in playerList)
			{
				text3 = text3 + (int)photonPlayer2.customProperties[(byte)225] + ":";
			}
			text3 = string.Concat(text3, PhotonNetwork.room.name + '|' + PhotonNetwork.room.customProperties["guid"]);
			ServiceManager.Instance.IsPrivateMatch = true;
			ServiceManager.Instance.SetMatchGameName(text3);
		...
		{
			ExitGames.Client.Photon.Hashtable hashtable2 = (ExitGames.Client.Photon.Hashtable)photonEvent[245];
			int num2 = (int)hashtable2[(byte)0];
	...
	public static void SendMonoMessage(PhotonNetworkingMessage methodString, params object[] parameters)
	{
		...
		{
			if (monoBehaviour != null && !hashSet.Contains(monoBehaviour.gameObject))
			{
