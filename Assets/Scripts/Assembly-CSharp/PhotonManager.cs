...
public class PhotonManager : MonoBehaviour
{
    ...
    public void CreateNetSyncObject(long netID, string objectType, int state, int ownerUserID, float posX, float posY, float posZ, float baseRotY, float angleRotX)
    {
        ...
        System.Type type = System.Type.GetType(objectType);
        if (type == null)
        {

            type = System.Type.GetType(objectType + ", Assembly-CSharp");
        }

        if (type == null)
        {
            Debug.LogError("Could not resolve NetSyncObject type: " + objectType);
            return;
        }


        NetSyncObject netSyncObject = userList[ownerUserID].gameObject.AddComponent(type) as NetSyncObject;
        if (netSyncObject == null)
        {
            Debug.LogError("The type " + objectType + " is not a NetSyncObject.");
            return;
        }
