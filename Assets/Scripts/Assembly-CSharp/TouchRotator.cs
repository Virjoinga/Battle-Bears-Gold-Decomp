...
public class TouchRotator : MonoBehaviour
{
	...
	private void Start()
	{
		currentRot = myTransform.parent.localEulerAngles.y;
	}
	...
	private void Update()
	{
		...
		{
			Touch aT = Input.touches[i];
			Ray ray = Camera.mainCamera.ScreenPointToRay(aT.position);
			RaycastHit hitInfo;
			if (myCollider.Raycast(ray, out hitInfo, 2000f))
			{
				...
				else if (touchID == aT.fingerId)
				{
					targetRot -= AnimatedScroller.FixTouchDelta(aT).x * 3f / ((!BBRQuality.HighRes) ? 1f : 2f);
				}
				flag = true;
			}
		}
	...
	private void LateUpdate()
	{
		if (!returnToZero)
		{
			if (Mathf.Abs(currentRot - targetRot) > 1350f)
			{
				Bootloader.Instance.unlockAchievement(GameCenterIDDictionaries.Achievements["DIZZY_DUMMY"]);
				if (Time.time > nextAllowedPukeTime && pukeSounds.Length > 0)
				{
		...
		myTransform.parent.localEulerAngles = new Vector3(0f, currentRot, 0f);
	}
}
